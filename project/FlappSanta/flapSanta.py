...
def create_tube():
    ...
    if score > 3 and score < 10:
        ...
        bot_tube = tube_screen.get_rect(midtop=(650, random_tube_pos))
        # ống trên
        top_tube = tube_screen.get_rect(midtop=(550, random_tube_pos - 650))
...
# check va chạm: khi vị trí của santa và vị trí của ống trùng nhau game dừng
# khi santa bay quá cao hay quá thấp cũng sẽ dừng
def collision(tubes):
    global can_score
    for tube in tubes:
...
def score_screen(state):
    ...
    # hiển thị điểm và điểm cao khi gameover
    if state == 'game over':
...
# check khi qua cột thì cộng điểm
def player_score():
    global score, can_score

    if tube_list:
        for tube in tube_list:
            if 95 < tube.centerx < 105 and can_score:  # nghĩa là tọa độ của ống vừa qua tọa độ của santa
                score += 1
                score_sound.play()
                can_score = False
...
def main():
    ...
    # tạo timer đổi trạng thái cho santa
    ...
    # tạo timer để ống xuất hiện sau 1 khoảng thời gian
    spawn_tube = pygame.USEREVENT
    pygame.time.set_timer(spawn_tube, 1700)  # sau 1.7 giây xuất hiện ống mới
